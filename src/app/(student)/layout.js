...
const NAV_MAIN = [
  { titleKey: "dashboard", href: "/student", icon: Home },
  { titleKey: "findMentors", href: "/student/preceptors", icon: Search },
  { titleKey: "myPlacements", href: "/student/placements", icon: ClipboardList, badge: 2 },
  { titleKey: "messages", href: "/student/messages", icon: MessageSquare, badge: 1 },
];

const NAV_ACCOUNT = [
  { titleKey: "profile", href: "/student/profile", icon: User },
  { titleKey: "credentials", href: "/student/credentials", icon: Upload },
  { titleKey: "preferences", href: "/student/preferences", icon: SlidersHorizontal },
  { titleKey: "availability", href: "/student/availability", icon: CalendarDays },
...
export default function StudentLayout({ children }) {
  ...
  const isOnHome = !currentItem || currentItem.href === "/student";
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <SidebarMenuButton size="lg" className="cursor-pointer">
                    <Avatar size="sm"><AvatarFallback className="bg-primary/10 text-primary text-xs">JS</AvatarFallback></Avatar>
                    <div className="grid flex-1 text-left text-sm leading-tight">
                      <span className="truncate font-medium">Jane Smith</span>
                      <span className="truncate text-xs text-muted-foreground">Nursing Student</span>
                    </div>
                  </SidebarMenuButton>
                </DropdownMenuTrigger>
                  ...
                  <DropdownMenuLabel className="font-normal">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="grid text-sm leading-tight">
                        ...
                        <span className="text-xs text-muted-foreground">Nursing Student</span>
                      </div>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuGroup>
                    <DropdownMenuItem asChild><Link href="/student/profile"><User className="size-4" />{tChrome("profile")}</Link></DropdownMenuItem>
                    <DropdownMenuItem asChild><Link href="/student/settings"><Settings className="size-4" />{tChrome("settings")}</Link></DropdownMenuItem>
        ...
        <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-3 border-b bg-background/95 px-4 backdrop-blur supports-backdrop-filter:bg-background/60">
          ...
          <Breadcrumb className="hidden sm:flex">
            <BreadcrumbList>
              {!isOnHome && (
                <>
                  <BreadcrumbItem><BreadcrumbLink asChild><Link href="/student">{tNav("dashboard")}</Link></BreadcrumbLink></BreadcrumbItem>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="relative">
                  ...
                  <span className="absolute -right-0.5 -top-0.5 flex size-4 items-center justify-center rounded-full bg-primary text-[10px] font-medium text-primary-foreground">2</span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-80">
                <DropdownMenuLabel className="flex items-center justify-between">{tChrome("notifications")} <Badge variant="secondary" className="text-xs">{tChrome("newCount", { count: 2 })}</Badge></DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuItem className="flex-col items-start gap-1 py-2.5">
                  <span className="text-sm font-medium">Placement request accepted</span>
                  <span className="text-xs text-muted-foreground">Dr. Williams accepted your placement request</span>
                  <span className="text-xs text-muted-foreground/60">1 hour ago</span>
                </DropdownMenuItem>
                <DropdownMenuItem className="flex-col items-start gap-1 py-2.5">
                  <span className="text-sm font-medium">New message</span>
                  <span className="text-xs text-muted-foreground">Dr. Garcia sent you a message</span>
